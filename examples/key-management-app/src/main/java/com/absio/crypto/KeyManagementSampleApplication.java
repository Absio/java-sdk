...
public class KeyManagementSampleApplication extends JFrame {
    ...
    private void computeHash() {
        try {
            byte[] hash = messageDigestHelper.digest(hashInput.getText().getBytes(CHARSET));
    ...
    private void genHMacDigest() throws UnsupportedEncodingException {
    ...
    private byte[] getHmacData() throws UnsupportedEncodingException {
        return hmacDataHexCheckBox.isSelected() ? DatatypeConverter.parseHexBinary(hmacData.getText()) : hmacData.getText().getBytes(CHARSET);
    }
    ...
    private byte[] getSignData() throws UnsupportedEncodingException {
        return signDataHexCheckBox.isSelected() ? DatatypeConverter.parseHexBinary(signData.getText()) : signData.getText().getBytes(CHARSET);
    }
    ...
    private void onHmacVerify() throws UnsupportedEncodingException {
        SecretKey key = getHmacKey();
        byte[] data = getHmacData();
    ...
    private void onIesEncrypt() {
        ...
        SwingWorker worker = new SwingWorker<byte[], Void>() {
            @Override
            protected byte[] doInBackground() throws UnsupportedEncodingException {
                byte[] plaintext;
                if (iesInputTextRadioButtonSelected) {
                    plaintext = iesInputTextText.getBytes(CHARSET);
                }
                else {
                    try {
    ...
    private void onKdf2GenKey() {
        ...
        try {
            if (kdfSecretTextRadioButton.isSelected()) {
                secret = kdfSecretText.getText().getBytes(CHARSET);
            }
            else {
    ...
    private void onSign() throws UnsupportedEncodingException {
    ...
    private void onVerify() throws UnsupportedEncodingException {
        byte[] data = getSignData();
        PrivateKey key = getSignaturePrivateKey();
        if (data == null || key == null) {
            return;
    ...
    private void pbkdf2Decrypt() {
        ...
        SwingWorker worker = new SwingWorker<byte[], Void>() {
            ...
            protected void done() {
                if (manualOutput) {
                    try {
                        KeyManagementSampleApplication.this.pdkdf2OutputManual.setText(new String(get(), CHARSET));
                    }
                    catch (Exception e) {
                        e.printStackTrace();
        ...
        SwingWorker worker = new SwingWorker<byte[], Void>() {
            ...
            protected byte[] doInBackground() throws Exception {
                ...
                if (StringUtils.isEmpty(saltTextText) && StringUtils.isEmpty(saltHexText)) {
                    ...
                    salt = saltTextText.getBytes(CHARSET);
                }
                else {
                ...
                try {
                    ...
                    if (decryptOutputFileRadioButtonSelected) {
                        ...
                        SwingUtilities.invokeLater(new Runnable() {
                            public void run() {
                                try {
                                    decryptOutputText.setText(new String(data, CHARSET));
                                }
